...
def test_startup_prompts_for_missing_jira_oauth2_refresh_token(
    jira_app_config: service_config.AppConfig, monkeypatch: pytest.MonkeyPatch
) -> None:
    reader_config = {"auth_type": "oauth2"}
    monkeypatch.setattr(service_config, "has_cached_refresh_token", lambda: False)
    monkeypatch.setattr(service_config, "run_jira_oauth_wizard", lambda: "refresh-token")
