...
2. **Environment variables** (for credentials only: `PASSWORD_HASH` and `SALT`)
3. **`config.toml`**
4. **Built-in defaults**
...
| `reader_config_path` | String | Path to a separate reader config file | `None` (uses inline `[...reader_config]`) |
| `host` | String | Host address to bind to | `"127.0.0.1"` |
| `port` | Integer | Port number to listen on | `8020` |
| `password_hash` | String | Hashed password (generated by `set-credentials`) | (none) |
| `salt` | String | Salt for password hashing (generated by `set-credentials`) | (none) |
...
You can also provide credentials via environment variables instead of the config file:
...
The following endpoints are reachable **without** authentication:
...
### Reader-specific authentication

Some readers have their own authentication requirements in addition to the service-level Basic Auth:

- **Jira reader**: requires Jira credentials (username + API token, PAT, or OAuth1). See [Jira configuration](readers/jira.md#authentication-methods).
- **JSONL / Excel readers**: no additional authentication needed (they read local files).
...
:::caution Windows limitation
When using mTLS (`ssl_ca_cert`), the service runs in single-process mode on Windows. For production mTLS, deploy on Linux or use proxy-terminated mTLS.
:::
...
| Option | Type | Description | Default |
|--------|------|-------------|---------|
| `proxies_count` | Integer | Number of proxy layers (for `X-Forwarded-For` depth) | `None` |
| `real_ip_header` | String | Custom header for client IP (e.g. `"X-Real-IP"`) | `None` |
| `forwarded_secret` | String | Secret token for RFC 7239 `Forwarded` header validation | `None` |
